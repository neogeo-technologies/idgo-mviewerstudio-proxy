...
def viewerstudio_store_user_content():
    ...
    if not os.path.exists(_map_directory):
        logging.warning("directory creation: {}".format(_map_directory))
        os.makedirs(_map_directory)
        logging.warning("directory creation done")

    if os.path.exists(_map_directory):
        logging.warning("directory exists: {}".format(_map_directory))
        logging.warning("directory stats before chmod: {}".format(str(os.stat(_map_directory))))
        os.chmod(_map_directory, mode=0o770)
        logging.warning("directory stats after chmod: {}".format(str(os.stat(_map_directory))))
